...
#include "stm32wb_usbdev.h"

#if defined(CONFIG_USBDEV) && defined(CONFIG_STM32WB_USB)
...
#    define STM32WB_IRQ_USB_HP   STM32WB_IRQ_USB_HP_2
#    define STM32WB_IRQ_USB_LP   STM32WB_IRQ_USB_LP_2
#    define STM32_IRQ_USBWKUP STM32_IRQ_USBWKUP_2
#  else
#    define STM32WB_IRQ_USB_HP   STM32WB_IRQ_USB_HP_1
#    define STM32WB_IRQ_USB_LP   STM32WB_IRQ_USB_LP_1
                            ...
                            uint16_t pma, uint16_t nbytes)
{
  ...
  dest = (uint16_t *)(STM32WB_USB1_PMAADDR + ((uint32_t)pma << 1));
  for (i = nwords; i != 0; i--)
    {
...
stm32_copyfrompma(uint8_t *buffer, uint16_t pma, uint16_t nbytes)
{
  ...
  src = (uint32_t *)(STM32WB_USB1_PMAADDR + ((uint32_t)pma << 1));
  for (i = nwords; i != 0; i--)
    {
...
#  endif
  ...
  if (irq_attach(STM32WB_IRQ_USB_HP, stm32_hpinterrupt, NULL) != 0)
    {
      usbtrace(TRACE_DEVERROR(STM32_TRACEERR_IRQREGISTRATION),
               (uint16_t)STM32WB_IRQ_USB_HP);
      goto errout;
    }

  if (irq_attach(STM32WB_IRQ_USB_LP, stm32_lpinterrupt, NULL) != 0)
    {
      usbtrace(TRACE_DEVERROR(STM32_TRACEERR_IRQREGISTRATION),
               (uint16_t)STM32WB_IRQ_USB_LP);
      goto errout;
    }
...
void arm_usbuninitialize(void)
{
  ...
  up_disable_irq(STM32WB_IRQ_USB_HP);
  up_disable_irq(STM32WB_IRQ_USB_LP);
  irq_detach(STM32WB_IRQ_USB_HP);
  irq_detach(STM32WB_IRQ_USB_LP);
...
int usbdev_register(struct usbdevclass_driver_s *driver)
{
  ...
  if (ret)
  ...
  else
    {
      ...
      up_enable_irq(STM32WB_IRQ_USB_HP);
      up_enable_irq(STM32WB_IRQ_USB_LP);
...
#define GPIO_USB_PULLUP (GPIO_OUTPUT|GPIO_OPENDRAIN|GPIO_SPEED_50MHz|\
                         GPIO_OUTPUT_SET|GPIO_PORTA|GPIO_PIN12)
int stm32_usbpullup(struct usbdev_s *dev, bool enable)
{
  usbtrace(TRACE_DEVPULLUP, (uint16_t)enable);
  stm32wb_gpiowrite(GPIO_USB_PULLUP, !enable);
  return OK;
}
